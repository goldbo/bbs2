...
<![endif]-->
<style> 
#leftList_left{ width:293px;}
#leftList_right{ overflow:auto;}
...
#listConTop{background:url(<%=baseURL%>/images/pnglist_top.png) top no-repeat; height:100%; overflow:auto;}
#listConTop h2{ font-size:16px; color:#355b84; margin:0px; padding:20px 0px 20px 40px;}
#listConTop span.titlespan{float:right; width:80px; font-size:12px;}
#listCon_right{float:right; width:310px; line-height:21px;height:100%;margin-right:20px}
...
<object id="VCITY_OCX" classid="clsid:D55D08CF-F692-4BA6-88D0-3C8A33373365" ></object>
<script type="text/javascript" src="<%=baseURL%>/vcity/inc/clientHandle.js"></script>
<script type="text/javascript">
function downloadHandle(type,id){
//<%=baseURL %>/vcityResourcesAction.do?action=downloadResource&id=${resource.id }
	if(type==1){
		installApplication(id);
	} else {
		installMobileModel(id);
	}
}
...
function showModel(moduleFileName,resurl,resid){
	...
	if(resurl!=""){
	ShowIframe('产品模型','<%=request.getContextPath()%>/vcity/produceOrder/produceOrder.jsp?file='+resurl+'&resid='+resid,630,630);
	}
	else {
...
function saveCom(){
	...
	vcityResourceForm.action = '<%=baseURL%>/vcityResourcesAction.do?action=showVcityResourceInfo&id=${resource.id }&resType=${resource.resType }';
	vcityResourceForm.submit();
	
}
</script>
<body>
<html:form action="/vcityResourcesAction.do?action=showVcityResourceInfo&id=${resource.id }&resType=${resource.resType }" styleId="vcityResourceForm" method="post" >
	...
	<div id="TopMenu">
		<div id="menuWrap">
			<ul>
				...
				<li  <c:if test="${resource.resType==2 }"> class="menu_on"</c:if><c:if test="${resource.resType==1 }"> class="menu_off"</c:if> onclick="location.href='<%=baseURL %>/vcityResourcesAction.do?action=showVcityResourcesList&resType=2';"><img src="<%=baseURL%>/images/icon_mobi.png" class="pngimg" /><br />手机模型</li>
				<li class="menu_line"></li>
				<li  <c:if test="${resource.resType==2 }"> class="menu_off"</c:if><c:if test="${resource.resType==1 }"> class="menu_on"</c:if> onclick="location.href='<%=baseURL %>/vcityResourcesAction.do?action=showVcityResourcesList&resType=1';"><img src="<%=baseURL%>/images/icon_app.png" class="pngimg" /><br />应用中心</li>
				<li class="menu_line"></li>
	...
	<div id="MListConWrap">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			...
			<div id="leftListTop" <c:if test="${resource.resType==2 }"> class="leftListTop2"</c:if><c:if test="${resource.resType==1 }"> class="leftListTop1"</c:if>>
				<div id="ConList">
					<ul>
						<c:if test="${!empty allDatas }">
						<c:forEach items="${allDatas }" var="res" varStatus="stauts" >
							<li><a title="${res.resName }" href="<%=baseURL %>/vcityResourcesAction.do?action=showVcityResourceInfo&id=${res.id }&resType=${res.resType }"><c:if test="${fn:length(res.resName) > 10}"> ${fn:substring(res.resName, 0, 10)}... </c:if>
						<c:if test="${fn:length(res.resName) <= 10}">  ${res.resName }</c:if></a></li>
						</c:forEach>
						</c:if>
					</ul>
				</div>
				...
				<c:forEach items="${hotList}" var="vcityResource" varStatus="stauts" begin="0" end="4" >
				  <tr>
					...
					<a class="imagehref" href="<%=baseURL %>/vcityResourcesAction.do?action=showVcityResourceInfo&id=${vcityResource.id }&resType=${vcityResource.resType }" ><img src="<%=baseURL%>/${vcityResource.fullImage }" border="0" onload="javascript:DrawImage(this,60,60)" /></a></td>
					<td class="border_line"><a href="<%=baseURL %>/vcityResourcesAction.do?action=showVcityResourceInfo&id=${vcityResource.id }&resType=${vcityResource.resType }" >${vcityResource.resName }</a></td>
		...
		<div id="leftListBottom"></div>
			...
			<div id="listConTop"  >
				<h2><span class="titlespan"><img src="images/icon_7.gif" align="absmiddle" />&nbsp;&nbsp;<a href="<%=baseURL %>/vcityResourcesAction.do?action=showVcityResourcesList&resType=${resource.resType }">返回</a></span>${resource.vcityResType.resType } ${resource.resName }</h2>
				...
				<br />
				 ...
				 <br /><br />
				 <c:if test="${!empty resource.moduleFileName }"><a onclick="downloadHandle(${resource.resType },${resource.id });" href="javascript:;"><img src="images/download.gif" border="0" /></a>&nbsp;&nbsp; </c:if> 
				</div>
				<div style=" margin-right:310px; padding-left:30px">
               	...
               	<c:if test="${resource.resType==2 }"><div style="padding:30px;line-height:18px"><b>如何应用：</b><br />1、下载皮肤后，请将皮肤复制到程序安装目录的XXX下<br />2、建议采用客户端下载，系统会自动安装皮肤 </div></c:if> 
              	<c:if test="${resource.resType==1 }"><div style="padding:30px;line-height:18px"><b>如何应用：</b><br />1、下载应用后，请将应用复制到程序安装目录的XXX下<br />2、建议采用客户端下载，系统会自动安装应用</div></c:if> 
				...
				<br />
			<!-- 点评 -->
			<table width="669" align="center" border="0" cellspacing="0" cellpadding="0" style="border-bottom:1px dashed #dadada;background:url('images/K_top.png') top ">
			  <tr>
