...
public class MemberService {
	...
	public String returnsearsql(UserForm userForm,boolean includeManager,String timeoffset){
		...
		if (!userGroups[0].equals("") && !userGroups[0].equals("all")) {
			...
			if(!extg.equals("or")){
				...
				sql.append(" (m.groupid in (" + userGroupIds.toString() + ") or m.extgroupids=17) ");
			}
		}
		if (!email.equals("")) {
			sql.append(where);
	...
	public List<Members> getAllMembersByExtgroupids(String extgroupids){
		return ((MembersDao) BeanFactory.getBean("memberDao")).getAllMembersByExtgroupids(extgroupids);
	}
}
