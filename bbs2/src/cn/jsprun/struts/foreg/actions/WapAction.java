...
public class WapAction extends BaseAction {
	...
	public ActionForward register(ActionMapping mapping, ActionForm form,
		...
		if(username==null||username.equals("")){
			...
			Map<String,String> map = dataBaseDao.executeQuery("SELECT COUNT(*) count FROM jrun_members").get(0);
	...
	public ActionForward goTo(ActionMapping mapping, ActionForm form,
		...
		if("last".equals(dow)){
			...
			try {
				...
				out.write("&#80;&#111;&#119;&#101;&#114;&#101;&#100;&#32;&#98;&#121;&#32;&#74;&#115;&#112;&#82;&#117;&#110;&#33;&#32;&#54;&#46;&#48;&#46;&#48;&#32;&#82;&#101;&#108;&#101;&#97;&#115;&#101;&#32;&#50;&#48;&#49;&#48;&#48;&#54;&#50;&#53;<br/>");
		...
		if(dow == null){
			...
			if(message.trim().equals("")){
				...
				if(modnewreplies){
					...
					dataBaseService.runQuery("update jrun_forums set lastpost='"+Common.addslashes(lastpost)+"',posts=posts+1, todayposts=todayposts+1 where fid='"+fid+"'",true);
					if(currentForum.get("type").equals("sub")){
						dataBaseService.runQuery("update jrun_forums set lastpost='"+Common.addslashes(lastpost)+"' where fid='"+currentForum.get("fup")+"'",true);
					}
			...
			if(searchid!=null){
				...
				List<Map<String,String>> tempMapList = dataBaseService.executeQuery("SELECT searchid, dateline, " +
						"('"+searchctrl+"'<>'0' AND "+(jsprun_uid==0 ? "useip='"+onlineip+"'" : "uid='"+jsprun_uid+"'")+" AND "+timestamp+"-dateline<"+searchctrl+") AS flood, " +
								...
								"WHERE ('"+searchctrl+"'<>'0' AND "+(jsprun_uid==0 ? "useip='"+onlineip+"'" : "uid='"+jsprun_uid+"'")+" AND "+timestamp+"-dateline<"+searchctrl+") OR (searchstring='"+searchstring+"' AND expiration>'"+timestamp+"') " +
										"ORDER BY flood");
				if(tempMapList!=null){
				...
				if(!searchindex.get("id").equals("0")){
					...
					if((srchfrom!=null&&!srchfrom.trim().equals(""))
						...
						if(srchuname!=null && !srchuname.trim().equals("")){
							...
							List<Map<String,String>> tempMapList3 = dataBaseService.executeQuery("SELECT uid FROM "+tablePre+"members WHERE username LIKE '"+srchuname.replace("_", "\\_")+"' LIMIT 50");
			...
			if(dow.equals("list")){
				...
				if(msgto == null){
					...
					dataBaseService.execute("INSERT INTO "+tablePre+"pms (msgfrom, msgfromid, msgtoid, folder, new, subject, dateline, message) " +
							"VALUES('"+jsprun_user+"', '"+jsprun_uid+"', '"+member.get("msgtoid")+"', 'inbox', '1', '"+subject+"', '"+timestamp+"', '"+message+"')");
				...
				if(pmid!=null){
					dataBaseService.execute("DELETE FROM "+tablePre+"pms WHERE pmid='"+pmid+"' AND msgtoid='"+jsprun_uid+"' AND folder='inbox'");
