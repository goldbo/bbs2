...
public class TagsAction extends BaseAction {
	...
	public ActionForward toThreadtags(ActionMapping mapping, ActionForm form,
		...
		if (name != null && !name.equals("")) {
			...
			List<Map<String,String>> threadtaglist = dataBaseService.executeQuery("select s.tid as ttid,t.*,f.name from jrun_threadtags as s left join jrun_threads as t on s.tid=t.tid left join jrun_forums as f on t.fid=f.fid where s.tagname='"+Common.addslashes(name)+"' limit "+start_limit+","+tpp);
