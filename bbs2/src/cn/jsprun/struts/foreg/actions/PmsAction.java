...
public class PmsAction extends BaseAction {
	...
	public ActionForward toView(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		if(folder!=null && folder.equals("announce")){
			...
			if(!Common.empty(readapmid)){
				...
				CookieUtil.setCookie(request, response, "readapmid", pmid, 15552000);
			}
		}else{
	...
	public ActionForward toSendPms(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		int uid = (Integer) session.getAttribute("jsprun_uid");
		if(uid!=0){
			String sql="select * from jrun_answertemplate";
			List<Map<String,String>> answertemplate=dataBaseService.executeQuery(sql);
			if(answertemplate!=null&&answertemplate.size()!=0){
				request.setAttribute("answertemplate",answertemplate);
			}
		}
	...
	public ActionForward sendPms(ActionMapping mapping, ActionForm form,
		...
		List<Map<String,String>> members = dataBaseService.executeQuery("select m.uid,m.username,m.newsletter,mm.ignorepm,u.maxpmnum from jrun_members m left join jrun_memberfields mm on m.uid=mm.uid left join jrun_usergroups as u on m.groupid=u.groupid where m.username='"+Common.addslashes(msgto.trim())+"'");
		if ((msgto != null && !msgto.equals("") && members != null && members.size()>0) || msgtobuddys != null) {
			...
			if (msgtobuddys != null) {
				for (int i = 0; i < msgtobuddys.length; i++) {
					List<Map<String,String>> memberslist = dataBaseService.executeQuery("select m.uid,m.username,m.newsletter,mm.ignorepm,u.maxpmnum from jrun_members m left join jrun_memberfields mm on m.uid=mm.uid left join jrun_usergroups as u on m.groupid=u.groupid where m.uid="+msgtobuddys[i]);
