...
public class MiscAction extends BaseAction {
	...
	public ActionForward pay(ActionMapping mapping, ActionForm form,
		...
		String paysumit  = request.getParameter("paysubmit");
		request.setAttribute("thread", thread);
		request.setAttribute("navigation", navigation);
		if(paysumit==null){
	...
	public ActionForward attachpay(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		int jsprun_uid = (Integer)session.getAttribute("jsprun_uid");
		Members member = (Members)session.getAttribute("user");
		String paysubmit = request.getParameter("paysubmit");
		Map<String, String> settings = ForumInit.settings;
		String creditstrans = settings.get("creditstrans");
		double netprice = 0;
	...
	public ActionForward debateumpire(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		String umpiresubmit = request.getParameter("umpiresubmit");
		if(umpiresubmit==null){
			request.setAttribute("tid", Integer.valueOf(tid));
			...
			List<Map<String,String>> bestbater = dataBaseService.executeQuery("select uid from jrun_members where username='"+bestdebater+"' limit 1");
			...
			dataBaseService.runQuery("UPDATE jrun_debates SET umpirepoint='"+Common.addslashes(umpirepoint)+"', winner='"+winner+"', bestdebater='"+bestdebater+"\t"+bestdebateruid+"\t"+bestdebaterstand+"\t"+bestdebatervoters+"\t"+bestdebaterreplies+"', endtime='"+timestamp+"' WHERE tid='"+tid+"'");
