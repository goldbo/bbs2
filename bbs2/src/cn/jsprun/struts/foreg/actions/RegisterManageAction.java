...
public class RegisterManageAction extends BaseAction {
	...
	public ActionForward register(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		HttpSession session = request.getSession();
		if (seccodeverify != null&&!seccodeverify.equals(session.getAttribute("rand"))) {
		...
		if (regstatus > 1) {
			if (regstatus == 2 &&invitecode.equals("")) {
				...
				List<Map<String,String>> invites=dataBaseService.executeQuery("SELECT uid,inviteip, expiration FROM jrun_invites WHERE invitecode='"+Common.addslashes(invitecode)+"' AND status IN ('1', '3')");
