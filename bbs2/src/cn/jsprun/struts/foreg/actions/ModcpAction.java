...
public class ModcpAction extends BaseAction {
	...
	public ActionForward editsubject(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		if(orig==null||orig.size()<=0){
			showMessage(null,getMessage(request, "thread_nonexistence"),response);
			return null;
		}
		...
		if(message!=null){
			showMessage(null,message, response);
			return null;
		}
		...
		if(result!=null){
			showMessage(null,result, response);
			return null;
		}
	...
	public ActionForward getip(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		String modertarhql = "SELECT m.adminid, p.first, m.regip ,p.fid,p.tid FROM jrun_posts p	LEFT JOIN jrun_members m ON m.uid=p.authorid WHERE pid='"+ pid + "' AND tid='" + tid + "'";
		...
		if(members==null || members.size()<=0){
			...
			Locale locale = getLocale(request);
			String address = seeker.getAddress(membermap.get("regip")==null?"":membermap.get("regip"),mr,locale);
