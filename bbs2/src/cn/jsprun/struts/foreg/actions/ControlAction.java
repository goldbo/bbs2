...
public class ControlAction extends BaseAction {
	...
	public ActionForward editInfo(ActionMapping mapping, ActionForm form,
		...
		if (typeid.equals("1")) {
			...
			String oldpass = md5.getLongToken(oldpassword);
			if (!oldpass.equals(member.getPassword())) {
				...
				if(newpassword!=null && !newpassword.equals("")){
					String newpass = md5.getLongToken(newpassword);
	...
	public ActionForward credits(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		if(creditssubmit!=null){
		...
		else if("changeande".equals(operation)){
			...
			if(submit!=null&&!"".equals(submit)){
				...
				int creditluntan=Integer.parseInt(request.getParameter("creditluntan"));
				if(pwd==null||"".equals(pwd)){
					request.setAttribute("errorInfo","请输入你的密码");
					return mapping.findForward("showMessage");
				}
				if(!md5.getLongToken(pwd).equals(member.getPassword())){
					request.setAttribute("errorInfo","你输入的密码错误,请重新输入");
					return mapping.findForward("showMessage"); 
				}
				Pattern pattern = Pattern.compile("[0-9]*"); 
				boolean isNum=pattern.matcher(credit).matches(); 
				if(!isNum){
					request.setAttribute("errorInfo","输入的兑换积分必须为数字");
					return mapping.findForward("showMessage");
				}
				if(credit.equals("0")){
					request.setAttribute("errorInfo","请输入你要兑换的积分");
					return mapping.findForward("showMessage");
				}
				if(creditluntan<Integer.parseInt(credit)){
					request.setAttribute("errorInfo","对不起,你没有那么多积分.请重新输入");
					return mapping.findForward("showMessage");
				}
				if(member==null){
					request.setAttribute("errorInfo","未知运行错误,请你退出后重新登录本系统");
					return mapping.findForward("showMessage");
				}
				int mid=member.getCuster_id();
				int point=Integer.parseInt(credit);
				String content=Common.getCredit(mid,(point/Integer.parseInt(settingMap.get("creditchange")))+"","论坛积分兑换成E声网积分");
				...
				if(!isok){
					try {
						request.setAttribute("errorInfo",new String(result.getBytes(),"utf-8"));
				...
				String updatepoint="UPDATE jrun_members SET credits=credits-"+credit+" WHERE uid="+member.getUid();
				dataBaseService.executeUpdateByHql(updatepoint);
				request.setAttribute("successInfo", "兑换成功");
				request.setAttribute("requestPath", "memcp.jsp?action=credits&operation=changeande");
				return mapping.findForward("showMessage");
			}
			else{
				if(member==null){
					request.setAttribute("errorInfo","对不起,你还未登录。");
					return mapping.findForward("showMessage");
				}
				else{
					request.setAttribute("creditchange", settingMap.get("creditchange"));
					String getCredit="SELECT credits FROM jrun_members WHERE uid="+member.getUid();
					List<Map<String,String>> credit=dataBaseService.executeQuery(getCredit);
					if(credit!=null&&credit.size()!=0){
						request.setAttribute("credit",credit.get(0));
					}
				}
			}
		...
		else if("changeande".equals(operation)||"transfer".equals(operation)||"exchange".equals(operation)&&exchangestatus>0||"addfunds".equals(operation)&&ec_ratio>0){
