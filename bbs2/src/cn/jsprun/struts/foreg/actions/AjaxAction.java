...
public class AjaxAction extends BaseAction {
	...
	public ActionForward checkusername(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if(Common.censoruser(username, censoruser)){
				...
				List<Map<String,String>> members=dataBaseService.executeQuery("SELECT uid FROM jrun_members WHERE username='"+Common.addslashes(username)+"' LIMIT 1");
	...
	public ActionForward checkinvitecode(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			List<Map<String,String>> invites=dataBaseService.executeQuery("SELECT invitecode FROM jrun_invites WHERE invitecode='"+Common.addslashes(invitecode)+"' AND status IN ('1', '3') LIMIT 1");
			PrintWriter out=response.getWriter();
