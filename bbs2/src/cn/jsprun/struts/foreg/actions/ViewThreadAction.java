...
public class ViewThreadAction extends BaseAction {
	...
	public ActionForward viewthreadpost(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		Map<String, String> settings = ForumInit.settings;//加载出的版块的相关信息
		HttpSession session = request.getSession();//得到会话
		int uid = (Integer)session.getAttribute("jsprun_uid");//得到登录用户的uid
		int tid =Common.toDigit(request.getParameter("tid"));//得到传递过来的主题的id
		String ajaxin = request.getParameter("inajax");//ajax
		//得到主题和主题所在的版块信息
		...
		Map<String,String> threads = threadpost.get(0);//得到第一条数据
		threadpost=null;
		...
		//查看专题帖时
		if("2".equals(threads.get("special"))){
			//得到该专题贴的权限
			String sqlgetPerm="select * from jrun_special where tid="+tid;
			List<Map<String,String>> listperm=dataBaseService.executeQuery(sqlgetPerm);
			if(listperm!=null){
				Map<String,String> mapperm=listperm.get(0);
				int perm=Integer.parseInt(mapperm.get("perm"));//专题帖权限
				if(perm==1&&uid==0){//当专题帖权限为会员，且当前用户为游客时
					request.setAttribute("show_message","对不起，你无权查看专题帖");
					return mapping.findForward("nopermission");						
				}
				else if(perm==3||perm==4){
					if(uid!=0){
						if(!threads.get("authorid").equals(uid+"")){
							String sqlgetPermlist="select * from jrun_permlist where account='"+members.getUsername()+"' and tid="+tid;
							List<Map<String,String>> mpl=dataBaseService.executeQuery(sqlgetPermlist);
							if(mpl==null||mpl.size()==0){
								request.setAttribute("show_message","对不起，你无权查看本专题帖");
								return mapping.findForward("nopermission");
							}
						}
					}
		...
		if(reply!=null&&reply!=""){
			request.setAttribute("onfocus","<script>document.getElementById('fastpostmessage').focus();</script>");
		}
		if(special==2){
			String specialAppend="Select * From jrun_special where tid="+tid;
			List<Map<String,String>> specialappend=dataBaseService.executeQuery(specialAppend);
			Map<String,String> specialMap=specialappend.size()>0?specialappend.get(0):null;
			if(specialMap!=null){
				request.setAttribute("specialappend", specialMap);
				int perm=Integer.parseInt(specialMap.get("perm"));
			}
			forwad="trade";
		}
		String comment="select * from jrun_comment where threadId="+tid;
		List<Map<String,String>> mcomment=dataBaseService.executeQuery(comment);
		if(mcomment!=null){
			int sumclicks=0;
            for(Map<String,String> list:mcomment){
            	sumclicks+=Integer.parseInt(list.get("clickTimes"));
            }
            request.setAttribute("clickssum", sumclicks);
			request.setAttribute("mcomment",mcomment);
			request.setAttribute("commentsize",mcomment.size());
		}
		else{
			request.setAttribute("clickssum", 0);
			request.setAttribute("mcomment",null);
			request.setAttribute("commentsize",null);
		}
	...
	private ActionForward viewthread_trade(HttpServletRequest request,HttpServletResponse response,ActionMapping mapping,Map<String,String>threads,Map<String,String>settings,Map<String,Map<String,String>> bbcodes,List<Map<String,String>> smilieslist,MessageResources resources,Locale locale,boolean modertar,int uid,Members member,boolean showavatars,boolean showsignatures,boolean threadpay,boolean allowgetattach,String extcreditname,boolean showimages,int readaccess,String ftpurl,String dateformat,String timeformat,Map creditstrans,String timeoffset,int tid,List<Map<String,String>>profiledlist,List<String>custom,String profileds,int highlightstatus,String highlight){
		...
		if(dos!=null){
			...
			if (dos.equals("viewtradelist")) {
				...
				String typeadd = !Common.empty(tradetypeid)?" and typeid="+tradetypeid+" ":"";
	...
	public ActionForward getSpecialList(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response){
		int nowdate=Common.dataToInteger(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()),8+"");
		String sql="select t.* from jrun_threads t inner join jrun_special s on t.tid=s.tid where t.speical=2 and (s.starttime<='"+nowdate+"' and s.endtime>'"+nowdate+"'";
		List<Map<String,String>> list=dataBaseService.executeQuery(sql);
		return null;
	}
	public ActionForward doThemeTemplate(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response){
		HttpSession session=request.getSession();
		...
		if("show".equals(option)){
			int page=curpage==null?1:curpage.equals("")?1:Integer.parseInt(curpage);
			String type=request.getParameter("type");
			request.setAttribute("type",type);
			request.setAttribute("curpage", page);
			List<ThemeTemplate> list=themeTemplateService.findThemeTemplate(type, member!=null?member.getUid():0,page);
			int count=themeTemplateService.findThemeTemplateCount(type, member!=null?member.getUid():0);
			...
			int temid=Integer.parseInt(request.getParameter("temid"));
			boolean success=themeTemplateService.deleteThemeTemplateById(temid);
			if(!success){
				request.setAttribute("show_message","删除失败");
				return mapping.findForward("nopermission");
			}
			else{
				try {
