...
public class ExtendsAction extends BaseAction {
	...
	public ActionForward getSevrerInfo_MemoryInfo(ActionMapping mapping,
		...
		request.setAttribute("message", getMessage(request, "a_extends_current_memroyinfo",String.valueOf((operatingSystemMXBean.getTotalPhysicalMemorySize()/1024/1024))));
		return mapping.findForward("message");
	}
	...
	public ActionForward orders(ActionMapping mapping,
		...
		if(ordersubmit!=null){
			...
			request.setAttribute("url_forward", "admincp.jsp?action=orders&searchsubmit=yes&orderstatus=&orderid=&users=&buyer=&admin=&sstarttime=&sendtime=&cstarttime=&cendtime=");
			return mapping.findForward("message");
		}else{
			...
			if(searchsubmit!=null){
				...
				if (orders != null && orders.size() > 0) {
					...
					for (Map<String,String> order : orders) {
						...
						switch (status) {
							case 1:
								order.put("status", getMessage(request, "a_extends_orders_search_status_pending"));
								break;
							case 2:
								order.put("status", "<b>"+getMessage(request, "a_extends_orders_search_status_auto_finished")+"</b>");
								break;
							case 3:
								order.put("status", "<b>"+getMessage(request, "a_extends_orders_search_status_manual_finished")+"</b><br />(<a href=\"space.jsp?action=viewpro&username="+Common.encode(order.get("admin"))+"\" target=\"_blank\">"+order.get("admin")+"</a>)");
								break;
						}
