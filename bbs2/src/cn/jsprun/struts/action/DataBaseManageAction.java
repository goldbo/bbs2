		...
		if (importsubmit != null || confirmed != null) {
			...
			if ("multivol".equals(dumpinfo.get("method"))) {
				...
				if ("1".equals(dumpinfo.get("volume"))) {
					...
					request.setAttribute("url_forward",request.getContextPath()+ "/database.do?action=importFile&from=server&datafile_server="+ datafile_next.toString()+ "&autoimport=yes&importsubmit=yes"+ (request.getParameter("delunzip") != null ? "&delunzip=yes": ""));
					return mapping.findForward("message");
				...
				if (zipFile.exists()) {
					...
					if (fileNames != null && fileNames.size() > 0) {
						...
						if (autoimport==null&&(volume == null || !"1".equals(volume))) {
							...
							request.setAttribute("url_forward",request.getContextPath()	+ "/database.do?action=importFile&from=server&datafile_server="+ fileName+ "&importsubmit=yes&delunzip=yes");
							return mapping.findForward("message");
					...
					if (autoimport != null) {
						...
						request.setAttribute("url_forward",request.getContextPath()+ "/database.do?action=importFile&from=server&datafile_server="+ datafile.toString()+ "&autoimport=yes&importsubmit=yesimportsubmit=yes&delunzip=yes");
