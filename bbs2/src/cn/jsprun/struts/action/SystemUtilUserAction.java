	...
	public ActionForward newlettersubmit(ActionMapping mapping,
		...
		if (sendvia.equals("email")) {
			...
			if (perta > 1) {
				...
				request.setAttribute("message", msg);
				request.setAttribute("url_forward", request.getContextPath()+ "/sysutil.do?useraction=newlettersubmit&updatecreditsubmit=yes&subject=settings&message=settings&sendvia=email&end=yes");
				return mapping.findForward("message");
			} else {
				session.removeAttribute("currsql");
				String msg = getMessage(request, "members_newsletter_succeed");
			...
			if (perta > 0) {
				...
				if(perta+beint>=count){
					...
					beint = beint+perta;
					request.setAttribute("message", msg);
					request.setAttribute("url_forward", request.getContextPath()+ "/sysutil.do?useraction=newlettersubmit&updatecreditsubmit=yes&subject=settings&message=settings&sendvia=pms&pertask="+perta+"&begin="+beint);
					return mapping.findForward("message");
				}
			}else{
				String msg = getMessage(request, "members_newsletter_succeed");
