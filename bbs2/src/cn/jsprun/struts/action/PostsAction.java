...
public class PostsAction extends BaseAction {
	...
	public ActionForward auditingNewThreads(ActionMapping mapping,
		...
		if(member.getAdminid()==3&&pf.getFid()<=0){
			...
			String currsql = "select count(*) from Threads as t where t.displayorder=" + displayorder+ " t.fid in ("+fids.substring(1)+")";
			String queryStr = "select t.tid, t.fid, t.author, t.authorid, t.subject, t.dateline,p.pid, p.message, p.useip, p.attachment from jrun_threads t LEFT JOIN jrun_posts p ON p.tid=t.tid WHERE t.displayorder=" + displayorder+ " t.fid in ("+fids.substring(1)+") ORDER BY t.dateline DESC ";
	...
	public ActionForward batchModthreads(ActionMapping mapping,ActionForm form, HttpServletRequest request,HttpServletResponse response) {
		...
		int[] idspecial=new int[audiArr.length];
		String[] threadname=new String[audiArr.length];
		for (int i = 0; i < audiArr.length; i++) {
			String postsValue = request.getParameter(audiArr[i]);
			if (postsValue != null) {
				if (postsValue.equals("validate")) {
					List<Map<String,String>> threadpostlist = dataBaseService.executeQuery("select t.tid,t.fid,t.subject,t.digest,t.authorid,t.author,t.dateline,p.anonymous,t.special from jrun_threads t left join jrun_posts p on t.tid=p.tid where p.first=1 and t.tid="+audiArr[i]);
					Map<String,String>threadpost = threadpostlist.get(0);
					idspecial[i]=Integer.parseInt(threadpost.get("special"));
					threadname[i]=threadpost.get("subject");
					threadpostlist = null;
	    ...
	    if(idspecial!=null){
	    	for(int i=0;i<idspecial.length;i++){
				if(idspecial[i]==2){
					String sqlGetPerm="Select * from jrun_special where tid="+validateList.get(i);
					List<Map<String,String>> listPerm=dataBaseService.executeQuery(sqlGetPerm);
					if(listPerm!=null){
						Map<String,String> mapperm=listPerm.get(0);
						if(mapperm.get("perm").equals("4")){
							String sqlGetEmail="Select account from jrun_permlist where tid="+validateList.get(i);
							List<Map<String,String>> account=dataBaseService.executeQuery(sqlGetEmail);
							String name="";
							if(account!=null){
								for(Map<String,String> maps:account){
									name=name+maps.get("account")+",";
								}
							}
							account=null;
							...
							List<Members> meb=this.getUserEmailFromE(name);
							 if(meb!=null&&meb.size()>0){
								for(Members mebs:meb){
									this.sendEmail(request, mebs.getEmail(), mebs.getUsername(),threadname[i]);
								}
							 }
//							String stremail="";
//							if(email!=null){
//								for(Map<String,String> mapemail:email){
//									stremail=stremail+mapemail.get("email")+",";
//								}
//							}
//							stremail=stremail.substring(0,stremail.length()-1);
//							SendMail(request, stremail);
						}
					}
				}
			}
	...
	public ActionForward pagePosts(ActionMapping mapping, ActionForm form,
		...
		if(member.getAdminid()==3&&pf.getFid()<=0){
			...
			String currsql = "select count(*) from Threads as t where t.displayorder=" + displayorder+ " AND t.fid in ("+fids.substring(1)+")";
			String queryStr = "select t.tid, t.fid, t.author, t.authorid, t.subject, t.dateline,p.pid, p.message, p.useip, p.attachment from jrun_threads t LEFT JOIN jrun_posts p ON p.tid=t.tid WHERE t.displayorder=" + displayorder+ " ADN t.fid in ("+fids.substring(1)+") ORDER BY t.dateline DESC ";
			ppf = new PostsPageForm(currsql,queryStr);
		}
		MessageResources resources = getResources(request);
		Locale locale = getLocale(request);
	...
	public ActionForward pageModreplies(ActionMapping mapping, ActionForm form,
		...
		if(member.getAdminid()==3&&pf.getFid()<=0){
			...
			String countsql = "select count(*) from Posts as p where p.first=0 and p.invisible="+invisible+" and p.fid in ("+fids.substring(1)+")";
	...
	public  boolean sendEmail(HttpServletRequest request,String email,String username,String taskname){
		...
		String htmlBody = InvitationTemplate.getEmailSpecialTemplate(request, username,taskname);
		Mail mail=new Mail(host, port, auth, auth_username, auth_password,"1");
		String alertmsg = mail.sendMessage(test_from, email, "《第一体验俱乐部》提醒：您有新的专题帖参加。",null, htmlBody);
		if(alertmsg==null){
			return true;
		}
		else{
