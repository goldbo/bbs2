...
public class OtherManageAction extends BaseAction {
		...
		if(announcesubmit == null && addsubmit == null){
			...
			if(announcements!=null&&announcements.size()>0){
				for(Map<String,String> announcement:announcements){
					...
					announcement.put("starttime",announcement.get("starttime"));
					announcement.put("endtime",announcement.get("endtime"));
				}
			}
			...
			if(!FormDataCheck.isValueString(subject)){
				request.setAttribute("message","请输入正确的标题");
				request.setAttribute("return", true);
				return mapping.findForward("message");
			}
			...
			if(!FormDataCheck.isValueString(message)){
				request.setAttribute("message","消息内容不能为空");
				request.setAttribute("return", true);
				return mapping.findForward("message");
			}
			...
			try {
				annBean.setStarttime(startTime);
				annBean.setEndtime(endTime);
			} catch (Exception e) {
			...
			if(otherSetService.addFormAnn(annBean)){
				...
				request.setAttribute("message","最新消息添加成功");
				request.setAttribute("url_forward", "admincp.jsp?action=announcements");
				return mapping.findForward("message");
			}else{
				request.setAttribute("message","最新消息添加失败");
				request.setAttribute("return", true);
				return mapping.findForward("message");
			}
		}
	...
	public ActionForward annedit(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		if(editsubmit == null){
			...
			if(announcements!=null&&announcements.size()>0){
				...
				request.setAttribute("message", "最新消息不存在");
				request.setAttribute("return", true);
				return mapping.findForward("message");
			}
		}else{
			...
			try {
				ann.setStarttime(newStartTime);
				ann.setEndtime(newEndTime);
			} catch (Exception e) {
				request.setAttribute("message", getMessage(request, "a_other_announcement_time_invalid"));
				request.setAttribute("return", true);
				return mapping.findForward("message");
	...
	public ActionForward faqlist(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		String faqsubmit = request.getParameter("faqsubmit");
		if(faqsubmit == null){
			boolean isfounder = (Boolean)request.getAttribute("isfounder");
			if(!isfounder){
				request.setAttribute("message", getMessage(request, "noaccess_isfounder"));
	...
	public ActionForward onlinelist(ActionMapping mapping, ActionForm form,
		...
		String onlinesubmit = request.getParameter("onlinesubmit");
	 ...
	 * @return
	 */
	public ActionForward doPmsTemplate(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response){
		...
		if("show".equals(option)){
			String sql="select a.*,m.username from jrun_answertemplate a,jrun_members m where a.creator=m.uid";
			List<Map<String,String>> answerTemp=dataBaseService.executeQuery(sql);
			if(answerTemp!=null&&answerTemp.size()!=0){
				request.setAttribute("ansertTemp",answerTemp);
			}
			return mapping.findForward("pmslist");
		}
		else if("delete".equals(option)){
			String[] ids=request.getParameterValues("id[]");
			if(ids==null||ids.length==0){
				request.setAttribute("message","请选择要删除的模板");
			...
			dataBaseService.runQuery("delete from jrun_answertemplate where id in("+id+")");
			try {
				request.getRequestDispatcher("admincp.jsp?action=pmsManage&option=show").forward(request,response);
	        ...
	        if(context!=null){
				...
				if("".equals(context)){
					request.setAttribute("message","模板内容不能为空");
					request.setAttribute("imgerror","jfkldsf");
					return mapping.findForward("message");
				}
				AnswerTemplate answer=new AnswerTemplate();
				answer.setContext(context);
				answer.setCreator(uid);
				answer.setCreattime(timestamp);
				boolean issuccess=answerTemplateService.addTemplate(answer);
				if(issuccess){
					try {
						request.getRequestDispatcher("admincp.jsp?action=pmsManage&option=show").forward(request,response);
					} catch (ServletException e) {
					...
					request.setAttribute("message","模板添加失败");
					request.setAttribute("imgerror","jfkldsf");
					return mapping.findForward("message");
				}
	        }
	        else{
	        	return mapping.findForward("pmsadd");
	        }
		}
			...
			if("toedit".equals(to)){
				if("".equals(id)){
					request.setAttribute("message","请选择要查看详情的记录");
					request.setAttribute("imgerror","jfkldsf");
					return mapping.findForward("message");
				}
				else{
					List<Map<String,String>> temp=dataBaseService.executeQuery("select * from jrun_answertemplate where id="+id);
					if(temp==null||temp.size()==00){
						request.setAttribute("message","没有找到该记录");
						request.setAttribute("imgerror","jfkldsf");
						return mapping.findForward("message");
					}
					request.setAttribute("temp",temp.get(0));
                    return mapping.findForward("pmsadd");
				}
			}
			else{
				...
				int creattime=Integer.parseInt(request.getParameter("creatTime"));
				AnswerTemplate answertemplate=new AnswerTemplate();
				answertemplate.setContext(context);
				answertemplate.setCreator(creator);
				answertemplate.setCreattime(creattime);
				answertemplate.setId(Integer.parseInt(id));
				boolean issuccess=answerTemplateService.updateTemplate(answertemplate);
				if(issuccess){
					try {
						request.getRequestDispatcher("admincp.jsp?action=pmsManage&option=show").forward(request,response);
					} catch (ServletException e) {
					...
					request.setAttribute("message","模板修改失败");
					request.setAttribute("imgerror","jfkldsf");
					return mapping.findForward("message");
				}
			}
			...
			request.setAttribute("message","没有定义操作");
			request.setAttribute("imgerror","jfkldsf");
			return mapping.findForward("message");
		}
		return null;
	...
	public ActionForward toGift(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response){
		String option=request.getParameter("option");
		if("add".equals(option)||"edit".equals(option)){
			String id=request.getParameter("gid");
			if(id!=null&&!"".equals(id)){
				List<Map<String,String>> present=dataBaseService.executeQuery("select * from jrun_present where id="+id);
				if(present==null||present.size()==0){
					request.setAttribute("message","所选的礼物不存在");
					request.setAttribute("imgerror","jfkldsf");
					return mapping.findForward("message");
				}
				request.setAttribute("present",present.get(0));
			}
			return mapping.findForward("togift");
		}
		return null;
	}
	 ...
	 * @return
	 */
	public ActionForward doManageGift(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response){
		HttpSession session=request.getSession();
		int uid=(Integer)session.getAttribute("jsprun_uid");
		String option=request.getParameter("option");
		if("show".equals(option)){
			int curpage=request.getParameter("curpage")==null?0:Integer.parseInt(request.getParameter("curpage"));
			String sql="select p.*,m.username from jrun_present p,jrun_members m where m.uid=p.creator ";
			String condition=request.getParameter("condition");
			if(condition!=null&&!"".equals(condition)){
				String type=request.getParameter("type");
				sql=sql+" and p."+type+" like '%"+condition+"%'";
				request.setAttribute("condition",condition);
				request.setAttribute("type",type);
			}
			List<Map<String,String>> count=dataBaseService.executeQuery(sql);
			int total=count.size()>0?count.size()%2==0?(count.size()/12):(count.size()/12+1):0;
			request.setAttribute("count",total);
			sql=sql+" order by p.creattime desc limit "+curpage*12+",12";
            List<Map<String,String>> list=dataBaseService.executeQuery(sql);
            if(list!=null&&list.size()>0){
            	request.setAttribute("presentlist",list);
            }
            request.setAttribute("curpage",curpage);
            return mapping.findForward("presentlist");
		}
		else if("add".equals(option)||"edit".equals(option)){
			...
			if("".equals(name)){
				request.setAttribute("message","礼物名不能为空");
				request.setAttribute("imgerror","jfkldsf");
				return mapping.findForward("message");
			}
			FileItem fileItem=fileupload.getFileItem("filepic");			
			String result=this.uploadImage(request, fileItem);
			if("add".equals(option)&&"no".equals(result)){
				request.setAttribute("message","礼物图片不能为空");
				request.setAttribute("imgerror","jfkldsf");
				return mapping.findForward("message");
			}
			if("error".equals(result)){
				request.setAttribute("message","礼物应该是图片类型");
				request.setAttribute("imgerror","jfkldsf");
				return mapping.findForward("message");
			}
			int timestamp = "add".equals(option)?(Integer)(request.getAttribute("timestamp")):Integer.parseInt(fileupload.getParameter("creatTime"));
			Present present=new Present();
			present.setCreator(uid);
			present.setCreatTime(timestamp);
			present.setMessage("".equals(message)?"":message);
			present.setName(name);
			present.setPresentUrl((fileItem!=null&&fileItem.getSize()>0)?result:fileupload.getParameter("url"));
			if("edit".equals(option))
			{
				present.setId(Integer.parseInt(fileupload.getParameter("id")));
			}
			boolean issuccess=presentService.saveOrUpdatePresent(present);
			if(issuccess){
				Common.requestforward(response, "admincp.jsp?action=giftManage&option=show");
			}
			else{
				request.setAttribute("message","add".equals(option)?"礼物添加失败":"礼物修改失败");
				request.setAttribute("imgerror","jfkldsf");
				return mapping.findForward("message");
			}
		}
	...
	public ActionForward doSendGift(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response){
		...
		if("send".equals(option)){
			String id=request.getParameter("id");
			if(id==null||"".equals(id)){
				request.setAttribute("message","请选择要赠送的礼物");
				request.setAttribute("imgerror","jfkldsf");
				return mapping.findForward("message");
			}
			String getAllGroup="select groupid,grouptitle,type from jrun_usergroups";
			List<Map<String,String>> grouplist=dataBaseService.executeQuery(getAllGroup);
			if(grouplist!=null&&grouplist.size()>0){
				request.setAttribute("grouplist",grouplist);
			}
			String getGift="select * from jrun_present where id="+id;
			List<Map<String,String>> present=dataBaseService.executeQuery(getGift);
			if(present!=null&&present.size()>0){
				request.setAttribute("present",present.get(0));
			}
			else{
				request.setAttribute("message","礼物不存在");
				request.setAttribute("imgerror","jfkldsf");
				return mapping.findForward("message");
			}
			return mapping.findForward("sendgift");
		}
		else if("tosend".equals(option)){
			HttpSession session=request.getSession();
			int uid=(Integer)session.getAttribute("jsprun_uid");
			
			String[] group=request.getParameterValues("group[]");
			if(group==null||group.length==0){
				request.setAttribute("message","请选择要发送的会员组");
				request.setAttribute("imgerror","jfkldsf");
				return mapping.findForward("message");
			}
			String message=request.getParameter("message");
			...
			String getuid="select m.uid from jrun_members m where m.groupid in("+groupid+") or m.extgroupids in("+groupid+")";
			List<Map<String,String>> listuid=dataBaseService.executeQuery(getuid);
			if(listuid!=null&&listuid.size()>0){
				for(Map<String,String> map:listuid){
					id.add(map.get("uid"));
				}
			}
			SendGifts sendgift=null;
			List<SendGifts> sendlist=new ArrayList<SendGifts>();
			if(id.size()>0){
				for(String rid:id){
					sendgift=new SendGifts();
	                sendgift.setMessage(message);
	                sendgift.setPresentId(gid);
	                sendgift.setReceiveId(Integer.parseInt(rid));
	                sendgift.setSendId(uid);
	                sendgift.setSendTime((Integer)(request.getAttribute("timestamp")));
	                sendlist.add(sendgift);
	                sendgift=null;
				}
			}
			boolean b=sendGiftsService.saveorupdate(sendlist);
			if(b){
				request.setAttribute("message", "礼物赠送成功,共赠送"+sendlist.size()+"件");
				request.setAttribute("url_forward", "admincp.jsp?action=giftManage&option=show");
				return mapping.findForward("message");
			}
			else{
				request.setAttribute("message","礼物发送失败组");
				request.setAttribute("imgerror","jfkldsf");
				return mapping.findForward("message");
			}
			...
			String hql = "select new cn.jsprun.vo.Sendgifts_sr(sg.id,p.name,sg.message,sg.sendTime,mm_1.username,mm_2.username)"+
		    "from SendGifts sg,Members mm_1 ,Members mm_2,Present p where sg.sendId=mm_1.uid and sg.receiveId=mm_2.uid and sg.presentId=p.id";
			int first=request.getParameter("curpage")==null?0:Integer.parseInt(request.getParameter("curpage"));
			List<Sendgifts_sr> list=sendGiftsService.getListByid(hql, first, 12);
			List<Sendgifts_sr> count=sendGiftsService.getListByid(hql, 0, 0);
			if(list!=null&&list.size()!=0){
				request.setAttribute("sendgiftlist", list);
			}
			int total=count.size()>0?count.size()%12==0?count.size()/12:count.size()/12+1:0;
			request.setAttribute("curpage", first);
			request.setAttribute("count", total);
			return mapping.findForward("giftslist");
		}
		else{
			request.setAttribute("message","没有定义操作");
			request.setAttribute("imgerror","jfkldsf");
