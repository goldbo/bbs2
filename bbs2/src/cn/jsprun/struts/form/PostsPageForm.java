...
public class PostsPageForm implements java.io.Serializable{
	...
	private String countSQL = "select count(*) from Threads as t where t.displayorder=-2 ";

	private String sql = "select t.tid, t.fid, t.author, t.authorid, t.subject, t.dateline,p.pid, p.message, p.useip, p.attachment from jrun_threads t LEFT JOIN jrun_posts p ON p.tid=t.tid WHERE t.displayorder=-2 ORDER BY t.dateline DESC ";

	private StringBuffer sb = new StringBuffer();
	public PostsPageForm(String fids) {
		if(!fids.equals("")){
			countSQL = "select count(*) from Threads as t where t.displayorder=-2 and t.fid in ("+fids.substring(1)+")";
			sql = "select t.tid, t.fid, t.author, t.authorid, t.subject, t.dateline,p.pid, p.message, p.useip, p.attachment from jrun_threads t LEFT JOIN jrun_posts p ON p.tid=t.tid WHERE t.displayorder=-2 AND t.fid in ("+fids.substring(1)+") ORDER BY t.dateline DESC ";
		}
		totalSize();
		getTotalPage();
	}
