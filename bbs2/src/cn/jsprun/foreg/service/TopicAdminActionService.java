...
public class TopicAdminActionService {
	...
	private String operating_deleteMirrorImage(Map<String,Object> parameterMap,Map<String,String> operationInfoMap,MessageResources mr,Locale locale){
		...
		for (Threads threads_old : threadsList) {
			...
			writeLog(parameterMap,currentForumName, threads_old.getSubject(), threads_old.getTid()
	...
	private String operating_move(Map<String,Object> parameterMap,Map<String,String> operationInfoMap,MessageResources mr,Locale locale){
		...
		DataBaseDao dataBaseDao = ((DataBaseDao)BeanFactory.getBean("dataBaseDao"));
		String sql_selectForum = "SELECT fid, name, modnewposts, allowpostspecial FROM "+tableprefix+"forums WHERE fid="+parameterMap.get("moveto")+" AND status>0 AND type<>'group'";
		List<Map<String,String>> resultList = dataBaseDao.executeQuery(sql_selectForum);
		String fid = (String)parameterMap.get("fid");
		String currentForumName = dataBaseDao.executeQuery("SELECT name FROM "+tableprefix+"forums WHERE fid="+fid).get(0).get("name");
		...
		if(expirationInteger==0){
			...
			StringBuffer hqlBuffer = new StringBuffer("UPDATE Forums AS f SET f.threads=f.threads-"+threadCount+", f.posts=f.posts-"+(replyCount+threadCount)+", ");
			if(lastPosts!=null){
				...
				if(lasThreads!=null){
					...
					hqlBuffer.append("f.lastpost='"+lastpost+"' ");
				}
			}else{
				hqlBuffer.append("f.lastpost='' ");
			}
			hqlBuffer.append("WHERE f.fid="+fid);
	...
	private void updateForumCount(Object fid,DataBaseDao dataBaseDao,MessageResources mr,Locale locale){
		...
		dataBaseDao.execute("UPDATE "+tableprefix+"forums SET posts='"+replycount+"', threads='"+threadcount+"', lastpost='"+lastpost+"' WHERE fid='"+fid+"'");
	}
